...
const parseBookingFromDB = (dbRow: any): Booking => {
    ...
    try {
        ...
        if (!camelCased.product) {
            ...
            camelCased.product = {
                id: camelCased.productId || `${productType}-fallback`,
                type: productType,
                name: fallbackName,
                ...
                details: {
                    ...
                    ...(dbRow.product_service_kind ? { serviceKind: dbRow.product_service_kind } : {}),
                    ...(dbRow.product_booking_source ? { bookingSource: dbRow.product_booking_source } : {}),
                }
            };
...
async function handleAction(action: string, req: VercelRequest, res: VercelResponse) {
    ...
    switch (action) {  // Corregido: Se agregó el paréntesis faltante
        ...
        case 'migrateDeliveryPhotosToBunny': {
            ...
            const requestedMaxDeliveries = Number(body.maxDeliveries);
            const batchSize = Number.isFinite(requestedBatchSize)
                ? Math.min(Math.max(Math.floor(requestedBatchSize), 1), 200)
                : 25;
            const maxDeliveries = Number.isFinite(requestedMaxDeliveries)
                ? Math.min(Math.max(Math.floor(requestedMaxDeliveries), 1), 1000)
                : batchSize;
            ...
            if (!dryRun && !bunnyEnabled) {
                return res.status(503).json({
                    ...
                    details: {
                        ...
                        hasCdnHostname: !!process.env.BUNNY_CDN_HOSTNAME
                    }
                });
            }

            type MigrationRow = { id: string; photos: any };
            ...
            const normalizePhotos = (rawPhotos: any): string[] => {
                ...
                if (!Array.isArray(parsed)) return [];
                return parsed
                    .filter((photo: any) => typeof photo === 'string')
                    .map((photo: string) => photo.trim())
                    .filter(Boolean);
            };

            const isUrl = (value: string): boolean => /^https?:\/\//i.test(value);
            const isDataUrlBase64 = (value: string): boolean => /^data:image\/[a-zA-Z0-9.+-]+;base64,/.test(value);
            const isRawBase64 = (value: string): boolean => value.length > 100 && /^[A-Za-z0-9+/=]+$/.test(value);
            const isBase64Candidate = (value: string): boolean => isDataUrlBase64(value) || isRawBase64(value);

            let queryRows: MigrationRow[] = [];
            const deliveryIds = Array.isArray(body.deliveryIds)
                ? body.deliveryIds.filter((id: any) => typeof id === 'string' && id.trim()).map((id: string) => id.trim())
                : [];
            ...
            if (deliveryIds.length > 0) {
                ...
                const query = `
                    ...
                    LIMIT $${deliveryIds.length + 1}
                `;
                const result = await sql.query(query, [...deliveryIds, maxDeliveries]);
                queryRows = result.rows as MigrationRow[];
            } else {
            ...
            const deliveriesToProcess = queryRows
                .map((row) => ({ id: String(row.id), photos: normalizePhotos(row.photos) }))
                .filter((row) => row.photos.some(isBase64Candidate))
                .slice(0, batchSize);

            const report = {
                scannedDeliveries: queryRows.length,
                candidateDeliveries: deliveriesToProcess.length,
            ...
            for (const delivery of deliveriesToProcess) {
                report.processedDeliveries += 1;
                const updatedPhotos: string[] = [];
                let deliveryBase64Count = 0;
                let deliveryMigratedCount = 0;
                ...
                for (let index = 0; index < delivery.photos.length; index += 1) {
                    ...
                    if (isUrl(photo)) {
                        report.alreadyUrlPhotos += 1;
                        updatedPhotos.push(photo);
                        continue;
                    }

                    if (!isBase64Candidate(photo)) {
                        report.skippedInvalidPhotos += 1;
                        updatedPhotos.push(photo);
                        continue;
                    }

                    deliveryBase64Count += 1;
                    ...
                    if (uploadResult.success && uploadResult.url) {
                        ...
                        deliveryMigratedCount += 1;
                        report.migratedPhotos += 1;
                    } else {
                        updatedPhotos.push(photo);
                        deliveryFailedCount += 1;
                        report.failedPhotos += 1;
                        const errorMsg = uploadResult.error || 'Unknown Bunny upload error';
                        deliveryErrors.push(`photo_${index + 1}: ${errorMsg}`);
                    }
                }

                const shouldUpdate = !dryRun && deliveryMigratedCount > 0;
                if (shouldUpdate) {
                    await sql`
                        UPDATE deliveries
                    ...
                    `;
                    report.updatedDeliveries += 1;
                }

                details.push({
                    deliveryId: delivery.id,
                    totalPhotos: delivery.photos.length,
                    base64Photos: deliveryBase64Count,
                    migratedPhotos: deliveryMigratedCount,
                    failedPhotos: deliveryFailedCount,
            ...
            return res.status(200).json({
                success: true,
                action: 'migrateDeliveryPhotosToBunny',
                bunnyEnabled,
                report,
                details
            });
        ...
        case 'approveGiftcardRequest': {
            ...
            try {
                ...
                try {
                    ...
                    console.warn('[approveGiftcardRequest] giftcard_events insert skipped:', eventErr);
                }
                ...
                const { rows: [freshRow] } = await sql`SELECT * FROM giftcard_requests WHERE id = ${id}`;
                const responsePayload: any = {
                    success: true,
                    request: toCamelCase(freshRow || updated),
                };
        ...
        case 'sendGiftcardNow': {
            ...
            try {
                ...
                if (!issuedCode) {
                    return res.status(400).json({ success: false, error: 'No hay código emitido. Aprueba la solicitud primero.' });
                }
