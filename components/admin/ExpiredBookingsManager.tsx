...
export const ExpiredBookingsManager: React.FC = () => {
  ...
  const [whatsappNotify, setWhatsappNotify] = useState<{ url: string; clientName: string; productName: string; date: string; time: string; participants: number; price: number; bookingCode: string } | null>(null);
  ...
  const handleConfirmPayment = async () => {
    ...
    const confirmedBookingId = payModal.bookingId;
    try {
      ...
      if (res.success) {
        ...
        // Construir notificación WhatsApp para el equipo
        ...
        if (confirmedBooking) {
          ...
          const dateStr = slot?.date
            ...
            : 'N/D';
          const timeStr = slot?.time || 'N/D';
          const participants = confirmedBooking.participants || 1;
          const finalPrice = confirmedBooking.price || amount;
          const waMsg = `*Pago Aprobado - CeramicAlma*\n\n` +
            `*Cliente:* ${clientName}\n` +
            `${confirmedBooking.userInfo?.email || ''}\n` +
            `*Tecnica:* ${productName}\n` +
            `*Fecha:* ${dateStr}\n` +
            `*Hora:* ${timeStr}\n` +
            `*Personas:* ${participants}\n` +
            `*Monto:* $${finalPrice.toFixed(2)}\n` +
            `*Codigo:* ${confirmedBooking.bookingCode}\n\n` +
            `*Estado:* Pago aprobado y verificado`;
          const waNumber = (process.env.VITE_COMPANY_WHATSAPP || '593985813327').replace(/\D/g, '');
          setWhatsappNotify({
            url: `https://wa.me/${waNumber}?text=${encodeURIComponent(waMsg)}`,
            clientName,
            productName,
            date: dateStr,
            time: timeStr,
      ...
      setActionLoading(p => { const n = { ...p }; delete n[payModal?.bookingId ?? confirmedBookingId]; return n; });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Modal WhatsApp Notificación al equipo */}
      {whatsappNotify && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setWhatsappNotify(null)}>
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center text-xl">✅</div>
              <div>
                <h3 className="text-lg font-bold text-brand-text">Pago aprobado</h3>
                <p className="text-xs text-brand-secondary">Notifica al equipo por WhatsApp</p>
              </div>
            </div>
            <div className="bg-brand-surface rounded-xl p-4 mb-5 space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-brand-secondary font-medium">Cliente</span>
                <span className="text-brand-text font-semibold">{whatsappNotify.clientName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-brand-secondary font-medium">Técnica</span>
                <span className="text-brand-text font-semibold">{whatsappNotify.productName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-brand-secondary font-medium">Fecha</span>
                <span className="text-brand-text font-semibold">{whatsappNotify.date}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-brand-secondary font-medium">Hora</span>
                <span className="text-brand-text font-semibold">{whatsappNotify.time}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-brand-secondary font-medium">Personas</span>
                <span className="text-brand-text font-semibold">{whatsappNotify.participants}</span>
              </div>
              <div className="flex justify-between border-t border-brand-border pt-2 mt-2">
                <span className="text-brand-secondary font-medium">Monto</span>
                <span className="text-green-700 font-bold">${whatsappNotify.price.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-brand-secondary font-medium">Código</span>
                <span className="font-mono text-brand-primary font-bold text-xs">{whatsappNotify.bookingCode}</span>
              </div>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => setWhatsappNotify(null)}
                className="flex-1 px-4 py-2 border border-brand-border rounded-lg text-brand-secondary hover:bg-brand-surface transition-colors text-sm"
              >
                Omitir
              </button>
              <a
                href={whatsappNotify.url}
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => setWhatsappNotify(null)}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg font-bold hover:bg-green-600 transition-colors text-sm"
              >
