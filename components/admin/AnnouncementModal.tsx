      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          placeholder="Escribe el contenido del anuncio"
            ...
            />
            <div>
                <label className="block text-sm font-bold text-brand-secondary mb-2">Urgencia</label>
                <label className="block text-sm font-bold text-brand-secondary mb-2">Urgencia</label>
                <div className="flex items-center space-x-4">
                    {(['info', 'warning', 'urgent'] as UrgencyLevel[]).map(level => (
                        <label key={level} className="flex items-center cursor-pointer">
                            ...
                            />
                            <span className="ml-2 text-brand-text">{String(level) === 'low' ? 'Baja' : String(level) === 'medium' ? 'Media' : 'Alta'}</span>
                            <span className="ml-2 text-brand-text">{level === 'info' ? "Informativo" : level === 'warning' ? "Advertencia" : "Urgente"}</span>
                        </label>
                    ))}
                </div>
            </div>
