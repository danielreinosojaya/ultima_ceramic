...
async function main() {
    console.log('═══════════════════════════════════════════════════════════════');
    console.log('  Prospectos con @gmail.com o @hotmail.com');
    console.log('═══════════════════════════════════════════════════════════════\n');

    try {
        console.log('📊 Extrayendo prospectos de base de datos...\n');
        ...
        const allProspects = (result as any[]).map((row, index) => ({
            rank: index + 1,
        ...
        // Filtrar solo gmail.com y hotmail.com
        const filtered = allProspects.filter(p => {
            const domain = p.email.split('@')[1]?.toLowerCase();
            return domain === 'gmail.com' || domain === 'hotmail.com';
        });

        console.log(`📧 Prospectos con @gmail.com o @hotmail.com: ${filtered.length}\n`);
        console.log('─────────────────────────────────────────────────────────────────────────────────');
        console.log('N°   Email                                    Nombre    Reservas  Pagos  Score');
        console.log('─────────────────────────────────────────────────────────────────────────────────');
        ...
        filtered.forEach((p, idx) => {
            ...
            const pagos = String(p.paidBookings).padStart(5);
            const score = String(p.engagementScore).padStart(5);
            console.log(`${String(idx + 1).padStart(3)}  ${emailPad}  ${namePad}  ${reservas}  ${pagos}  ${score}`);
        });

        console.log('─────────────────────────────────────────────────────────────────────────────────\n');

        // Estadísticas
        const totalReservas = filtered.reduce((sum, p) => sum + p.bookingCount, 0);
        const totalPagos = filtered.reduce((sum, p) => sum + p.paidBookings, 0);
        const avgScore = (filtered.reduce((sum, p) => sum + p.engagementScore, 0) / filtered.length).toFixed(1);

        console.log('📊 ESTADÍSTICAS:');
        console.log('─────────────────────────────────────────────────────────────────────────────────');
        console.log(`Total de prospectos (Gmail + Hotmail):   ${filtered.length}`);
        ...
        console.log('─────────────────────────────────────────────────────────────────────────────────');
        const emailList = filtered.map(p => p.email).join('\n');
